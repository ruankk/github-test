  ...
  */
  ...
  BEGIN
    ...
    if is_exists <> 0 then
      ...
      --调用饶文佳的请求包 批量执行出库操作
      V_OUT_MAG := '';
      cux_spm_data_po_pkg1.commit_inv_request_new(V_ORG_ID, V_OUT_MAG);
    
      --如果请求返回成功
      --以接口表为准，成功后会删除
      --IF V_OUT_MAG = '物料事务处理成功' THEN
      UPDATE SPM_CON_ODO_DL O
         SET O.Sync_Status = 'S', O.LAST_UPDATE_DATE = SYSDATE
       WHERE 1 = 1
      ...
      FOR REC_3 IN CUR_3(P_BATCH_ID) LOOP
        ...
        ELSE
          UPDATE SPM_CON_ODO O
             SET O.ATTRIBUTE5 = 'Y', O.LAST_UPDATE_DATE = SYSDATE
           WHERE O.ODO_ID = REC_3.ODO_ID;
        END IF;
