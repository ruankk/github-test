...
CREATE OR REPLACE PACKAGE BODY "SPM_CON_VENDOR_PACKAGE" is
  ...
  */
  ...
  begin
    ...
    select f.rmb_total, to_number(f.contract_version), f.contract_flag
      into nmoney, version, flag
      from spm_con_ht_info f
     where f.contract_id = ht_id;
    ...
    select f.rmb_total
      into bmoney
      from spm_con_ht_info f
     where f.contract_flag = flag
       and f.contract_version = to_char(version - 1);
